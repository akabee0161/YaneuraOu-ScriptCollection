...
`コマンド受付を開始しました。` と表示されるまでは、`peta_shock`、`peta_shock_latest`、`peta_read`、`peta next`、`enqueue`、`DB手動保存` などの操作ボタンは無効です。
...
既存の定跡DBを peta shock 化して leaf を延長する場合は、BookMiner 上で変換するなら `peta_shock` または `peta_shock_latest`、別マシンなどで変換済みの `peta_book-....db` または `.ybb` を持ち込むなら `peta_read` のあとに `peta next`、`peta refutation`、`peta rejoin`、`peta unsolved`、`peta opponent` のいずれかを使います。
...
1. `棋譜抽出`、または `peta_shock` / `peta_shock_latest` / 外部変換後の `peta_read` → `peta next`、`peta refutation`、`peta rejoin`、`peta unsolved`、`peta opponent`
2. `enqueue`
...
`棋譜抽出` は KifManager を起動します。棋譜抽出結果として `book/think_sfens.txt` ができるので、この場合は `peta_shock`、`peta next`、`peta refutation`、`peta rejoin`、`peta unsolved`、`peta opponent` を実行せずに `enqueue` へ進みます。
...
`peta_shock_latest` は `pl` コマンドを送信します。現在の定跡 DB は保存せず、`book/backup/` にある最新の通常bookを peta shock 化して読み込みます。直前の自動保存や手動保存で十分新しい通常bookがすでにあるとき、二重保存を避けるために使います。メモリ上にだけ存在する未保存の局面は、この変換には含まれません。
...
- `peta_shock_latest`: DBを書き出さず、`book/backup/` の最新通常bookを peta shock 化して読み込みます。
...
- `petaログ`: `peta next` / `peta refutation` / `peta rejoin` / `peta unsolved` / `peta opponent` の出力、`peta_shock` / `peta_shock_latest` の変換ログ、`peta_read` の読み込みログを表示します。
...
`定跡読込` の行に現在の定跡局面数が表示され、その下に採掘速度、平均探索 nodes、現在の nps も表示されます。

```text
現在の採掘速度 124,567 局面/日    平均探索nodes 1,000,000    現在のnps 1,441,748
```
...
GUI は BookMiner.py / BookMinerCpp から送られてくる累積値を 1 分ごとに記録し、直近 10 分の増加分から 1 日あたりの増加局面数を推定します。
平均探索 nodes も直近 10 分の `go nodes` 指定値の合計を、同じ期間に実際に探索した局面数で割って表示します。
現在の nps は、直近 10 分の book 追加局面数/秒に平均探索 nodes を掛けた見かけの値です。
起動直後など、まだ十分なサンプルがない間は速度、平均探索 nodes、現在の nps が `-` と表示されます。
...
[MiningProgress] positions=12345678 searched_positions=1234 nodes=1234000000 avg_nodes=1000000
[PetaCommandDone]
[PetaReadDone]
[PetaNextDone] path=book/think_sfens.txt count=50000
[PetaRejoinDone] path=book/think_sfens.txt count=1200
```
...
手動で `peta_shock`、`peta_shock_latest`、`peta_read`、手順2の各 peta 操作、`enqueue`、`DB手動保存` を実行している間は、自動enqueueは開始しません。
